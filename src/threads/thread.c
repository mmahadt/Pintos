...
/* List of processes in THREAD_BLOCKED state, that is, processes
   that are blocked unless they are waken up by a trigger (event). */
...
thread_init (void)
{
  ...
  list_init (&blocked_list);//initialize an empty blocked-threads list
  list_init (&all_list);
...
thread_block (void)
{
  ...
  list_push_back( &blocked_list, &thread_current ()->blockelem);
...
/* Invoke function 'func' on all BLOCKED threads, passing along 'aux'.
   This function must be called with interrupts off. */
void
thread_foreachblocked (thread_action_func *func, void *aux)
{
  struct list_elem *e;
